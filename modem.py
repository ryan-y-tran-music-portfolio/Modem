...
def samples_to_bits(
    samples: np.ndarray, samples_per_bit: int, sample_rate: int
) -> list[int]:
    ...
    for i in range(0, len(samples) - samples_per_bit + 1, samples_per_bit):
        ...
        print(
            f"Power Space: {power_space} | Mark Space: {mark_space} | Bit: {bit}"
        )  # Debug
        bits.append(bit)
...
def decode_wavfile(file_path: str) -> str:
    ...
    try:
        ...
        if sample_rate != 48000 or data.ndim > 1:
            raise Exception(
                f"{file_path} should be a 48 kilosample per second 16-bit mono WAV file."
            )
